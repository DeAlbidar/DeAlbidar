...
class Index_Model extends Model {
    ...
    public function contact($data) {
        $stmt = $this->db->insert('contact', array(
            'name' => Libs::Input($data['name']),
            'email' => Libs::Input($data['email']),
            'subject' => Libs::Input($data['subject']),
            'message' => Libs::Input($data['message']),
            'created_at' => date("Y-m-d H:i:s")
        ));     
        if($stmt){
            Libs::Email('albidar@w3-multimedia.com', 'Albidar W3 Multimedia', Libs::Input($data['name']), Libs::Input($data['message']));
        }
    }
